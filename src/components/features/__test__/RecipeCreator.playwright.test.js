...
test.describe('Botón Guardar Receta en RecipeCreator', () => {
  test('El botón aparece solo cuando hay receta generada y guarda en localStorage', async ({ page }) => {
  await page.goto('http://localhost:3000/recetia-app/#/');

  // Ingresa la API Key en el campo correspondiente y envía el formulario
  await page.waitForSelector('input[aria-label="Ingresa tu API Key de Gemini"]', { timeout: 10000 });
  await page.getByLabel('Ingresa tu API Key de Gemini').fill('AIzaSyB6c-Uj8MSovr_jEEt0hhSK96LzAhUF-Ok');
  await page.getByRole('button', { name: /Guardar/i }).click();

  // Espera a que se redirija a la vista principal
  await page.waitForSelector('input[aria-label="Ingresa un ingrediente"]', { timeout: 10000 });
  ...
  const savedRecipeVisible = await page.locator('.saved-recipes-list').getByText(/pollo/i).isVisible();
